...
def test_find_links_trailing_punctuation_and_parentheses():
    ...
    assert any("youtu.be/xyz" in l for l in links)
    assert any("instagram.com/p/ABC123" in l for l in links)
...
def test_find_links_case_insensitive_scheme():
    ...
    assert any("youtu" in l.lower() for l in links)

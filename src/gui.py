...
def _check_admin(request: Request) -> bool:
    ...
    if session and session.get('user'):
        ...
        if isinstance(user, dict):
            email = user.get('email') or user.get('preferred_username') or user.get('sub')
        if email:
...
async def gui_config():
    return JSONResponse({'oauth_configured': 'provider' in oauth._clients, 'admin_token_set': bool(os.environ.get('ADMIN_TOKEN'))})
...
async def get_requests(request: Request, limit: int = 50, offset: int = 0):
    ...
    for r in rows:
        ...
        (rid, chat_id, url, status, created_at, description,
         original_message_id, original_size, final_size, compressed,
         processing_started_at, processing_finished_at, processing_duration_seconds) = r
        def _mb(b):
            try:
        ...
        try:
            ...
            for ev in ev_rows:
                events.append({"id": ev[0], "type": ev[1], "details": ev[2], "duration_seconds": ev[3], "created_at": ev[4]})
        except Exception:
            events = []
        results.append({
            "id": rid,
    ...
    return JSONResponse({"total": total, "offset": offset, "limit": limit, "items": results})
...
async def api_stats(request: Request):
    ...
    try:
        cur.execute("SELECT COUNT(DISTINCT request_id) FROM request_events WHERE event_type IN ('compress','redownload')")
...
async def api_list_users(request: Request, limit: int = 50, offset: int = 0):
    ...
    results = [{"id": r[0], "username": r[1], "email": r[2], "role": r[3], "created_at": r[4]} for r in rows]
...
async def api_create_user(request: Request):
    ...
    return JSONResponse({"id": user_id, "username": username, "email": email, "role": role})
        ...
        for r in recent_requests:
            ...
            def _mb(b):
                try:
                    ...
                    return None
            reqs.append({
                'id': rid, 'chat_id': chat_id, 'url': url, 'status': status, 'created_at': created_at,
                'description': description, 'original_message_id': original_message_id,
                'original_size_mb': _mb(original_size), 'final_size_mb': _mb(final_size), 'compressed': bool(compressed) if compressed is not None else None,
                'processing_started_at': processing_started_at, 'processing_finished_at': processing_finished_at, 'processing_duration_seconds': processing_duration_seconds
            })
        await websocket.send_json({"type": "initial", "requests": reqs})
        recent_updates = db.list_updates(limit=50, offset=0)
        await websocket.send_json({"type": "initial_updates", "updates": [{"id": r[0], "raw": r[1], "created_at": r[2]} for r in recent_updates]})
